...
if st.button("Add Transaction"):
    ...
    new_entry = {
        ...
        "Debit (Outflow)": debit if debit else "",
        "Credit (Inflow)": credit if credit else "",
        "Balance": new_balance
    }
...
total_debit = st.session_state.ledger["Debit (Outflow)"].replace("", 0).astype(float).sum()
total_credit = st.session_state.ledger["Credit (Inflow)"].replace("", 0).astype(float).sum()
...
if len(st.session_state.ledger) > 0:
    ...
    subtotal_df["Debit (Outflow)"] = subtotal_df["Debit (Outflow)"].replace("", 0).astype(float)
    subtotal_df["Credit (Inflow)"] = subtotal_df["Credit (Inflow)"].replace("", 0).astype(float)
